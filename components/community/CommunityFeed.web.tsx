...
export default function CommunityFeed({ customHeader }: CommunityFeedProps = {}) {
  ...
  // WEB FIX: Use useRef for stable reference to prevent FlatList error
  // "Changing onViewableItemsChanged on the fly is not supported"
  const viewabilityConfig = useRef({ 
    itemVisiblePercentThreshold: 60 
  });

  // WEB FIX: Store callback in ref to maintain stable reference
  const onViewableItemsChangedRef = useRef(({ viewableItems }: { viewableItems: ViewToken[] }) => {
    ...
    viewableItems.forEach((viewToken) => {
      ...
      if (item && item.user_id !== currentUser.id) {
        useCommunityStore.getState().recordView(item.id, currentUser.id);
      }
    });
  });
